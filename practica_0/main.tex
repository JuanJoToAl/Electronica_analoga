...
\title{Informe práctica 0: Manejo de Equipos de Medición en Electrónica Análoga}
\author{
    \IEEEauthorblockN{Juan José Tobar Álvarez}{\textit{ jutobara@unal.edu.co}} \\
    \and
    \IEEEauthorblockN{David Alejandro Montes Rodríguez}{\textit{ dmontesr@unal.edu.co}} \\
    \and
    \IEEEauthorblockN{Jean Sebastian Claros Araujo}{\textit{ jclarosa@unal.edu.co}} \\
}
\begin{document}
\maketitle
...
\subsection{Modelado de Equipos e Impedancias}
De acuerdo con la guía de laboratorio \cite{guia_practica0}, y en relación con los diagramas esquemáticos empleados para los equipos, es pertinente dar respuesta a los siguientes interrogantes teóricos:

\textbf{¿Qué representa la región punteada en los circuitos?} \\
La región punteada representa el modelo eléctrico equivalente (basado en el Teorema de Thévenin \cite{sedra2020}) del interior del equipo de medición, como ocurre con el generador de señales. Físicamente, esta región delimita y aísla los componentes teóricos internos e ideales del instrumento (su fuente de tensión propia y su red resistiva) del circuito externo que conecta el usuario en los terminales. Todo lo que se encuentra dentro del recuadro punteado es inherente a la construcción del equipo.

\textbf{¿A qué corresponden los valores indicados como $R\_g$?} \\
El valor $R_g$ corresponde a la impedancia interna equivalente del instrumento de medición. Para un generador de señales, $R_g$ representa su impedancia de salida ($Z_{out}$, típicamente $50\,\Omega$), la cual se encuentra en serie con la fuente ideal y provoca caídas de voltaje internamente al acoplar una carga. En el caso del osciloscopio, $R_g$ representaría su impedancia de entrada ($Z_{in}$), la cual está diseñada para ser extremadamente alta (usualmente $1\,\text{M}\Omega$) con el objetivo de evitar extraer corrientes perjudiciales y causar el denominado efecto de carga sobre el circuito a medir \cite{sedra2020, buadlab_impedance}.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\columnwidth]{medicion voltaje de resistencia.jpeg}
\caption{Medición del voltaje de resistencia.}
\label{fig:medicion voltaje de resistencia}
\end{figure}
...
Al contrastar los valores teóricos de la Sección II-A con las mediciones experimentales del Cuadro~III, se observa una concordancia muy alta, resumida en el Cuadro~\ref{tab:mediciones_dc}. Las tensiones medidas en $R_1$, $R_2$, $R_3$ y $R_4$ (2,1~V, 1,96~V, 2,84~V y 0,87~V) se desvían de las calculadas (2,170~V, 1,945~V, 2,830~V y 0,884~V) en 3,2\,\%, 0,8\,\%, 0,4\,\% y 1,6\,\%, respectivamente. Las corrientes medidas (2,18~mA, 0,90~mA, 1,31~mA y 0,91~mA), frente a las calculadas (2,170~mA, 0,884~mA, 1,286~mA y 0,884~mA), presentan errores de 0,5\,\%, 1,8\,\%, 1,9\,\% y 2,9\,\%. Ninguna de las magnitudes supera el 3,2\,\% de error, un margen coherente con la tolerancia de $\pm 10\%$ de las resistencias empleadas, ya que los cálculos se realizaron con valores nominales y no con los valores reales de cada componente. Adicionalmente, las mediciones cumplen las leyes de Kirchhoff. La corriente medida en $R_2$ (0,90~mA) es prácticamente igual a la de $R_4$ (0,91~mA), como corresponde a dos elementos en serie. La suma de las corrientes de $R_2$ y $R_3$ (2,21~mA) es cercana a la corriente total entregada por la fuente (2,18~mA). Además, la tensión en $R_3$ (2,84~V) coincide con la suma de las caídas en $R_2$ y $R_4$ (2,83~V). La mayor desviación relativa se presenta en la tensión de $R_1$ (3,2\,\%) y en la corriente de $R_4$ (2,9\,\%), lo cual puede atribuirse a la tolerancia de las resistencias, a la resolución limitada de los multímetros y a la resistencia interna del amperímetro, que al conectarse en serie modifica ligeramente el circuito bajo prueba. También pueden influir las resistencias de contacto de la protoboard al desconectar y reconectar cada rama durante las mediciones.
...
\subsection{Comparación de los valores RMS calculados y medidos para distintas formas de onda}



Al comparar el Cuadro~IV con los valores calculados en la Sección II~B, se observan dos comportamientos distintos.
El osciloscopio reproduce el $V_{RMS}$ calculado (2,031~V, 3,905~V, 4,619~V y 5,138~V) con errores de 3,0\,\%, 2,7\,\%, 0,5\,\% y 0,6\,\%, ya que procesa la señal completa, incluido su nivel DC.
Los multímetros, en cambio, entregan lecturas entre 15\,\% y 52\,\% inferiores, porque en su función de tensión alterna descartan el nivel continuo calculado (1~V, 3~V, $-4$~V y 3,6~V) y miden solo la componente AC, $V_{RMS,AC}=\sqrt{V_{RMS}^2-V_{DC}^2}$.
Frente a este valor (1,768~V, 2,500~V, 2,309~V y 3,666~V), el multímetro True RMS presenta errores de apenas 2,4\,\%, 0,6\,\%, 0,3\,\% y 4,5\,\%, lo que confirma esta interpretación.

El multímetro de respuesta promedio mide el valor medio rectificado de la componente AC y lo multiplica por el factor de forma de una senoide (1,11).
Con este procedimiento se esperan 1,768~V, 2,775~V, 2,22~V y 3,73~V, frente a los 1,728~V, 2,759~V, 2,226~V y 4~V medidos, con errores de 2,3\,\%, 0,6\,\%, 0,3\,\% y 7,2\,\%.
Por eso solo coincide con el True RMS en la onda senoidal; en la cuadrada lo sobreestima en un 11\,\% y en la triangular lo subestima en un 3,3\,\%.
Por tanto, solo el osciloscopio entrega el valor eficaz total de señales con componente DC, mientras que en los multímetros es necesario conocer su modo de acoplamiento y su tipo de respuesta para interpretar la lectura.


\subsection{Mediciones RMS y DC con variación de la frecuencia}

Del ítem 3.1.1 se escogió la señal seno con 5 Vpp, nivel DC de 1 V y una frecuencia de 500 Hz, posteriormente se realizaron mediciones RMS y DC con ambos multímetros y el osciloscopio, cabe recalcar que el osciloscopio mide el valor DC calculando el promedio matemático de la señal en el tiempo. Como los semiciclos positivos y negativos de la onda senoidal se cancelan exactamente entre sí, el valor promedio total de la señal corresponde únicamente al offset aplicado. Por lo tanto, la medición DC registra estrictamente los 1 V de corriente continua.

Los puntos de frecuencia se escogieron por distribución logarítmica.

\begin{table}[H]
\centering
\caption{Mediciones RMS y DC con variación de la frecuencia (sinusoidal 5 Vpp, 1 V DC)}
\label{tab:rms_dc_frecuencia}
\begin{tabular}{|l|l|l|l|l|}
\hline
\textbf{Frecuencia} & \textbf{TRMS (V)} & \textbf{DC (V)} & \textbf{Osciloscopio (V)} & \textbf{Punto} \\
\hline
10 & 1,716 & 1,719 & 2,024 & 1 \\
\hline
35 & 1,762 & 1,77 & 2,094 & 2 \\
\hline
129 & 1,766 & 1,774 & 2,045 & 3 \\
\hline
464 & 1,763 & 1,763 & 1,768 & 4 \\
\hline
1,7 k & 1,724 & 1,689 & 2,094 & 5 \\
\hline
6 k & 0,637 & 1,117 & 2,027 & 6 \\
\hline
22 k & 0,024 & 0,26 & 2,039 & 7 \\
\hline
77 k & 0 & 0,003 & 2,051 & 8 \\
\hline
280 k & 0 & 0,002 & 2,085 & 9 \\
\hline
1 M & 0,02 & 0,002 & 2,033 & 10 \\
\hline
\end{tabular}
\end{table}

\subsection{Comparación de resultados calculados y medidos}

Como se mencionó anteriormente el $V_{RMS}$ de la señal escogida es 2,031 V y el DC es 1 V. Los valores medidos con el osciloscopio (2.02 V -- 2.09 V) se mantienen prácticamente constantes en las 10 frecuencias y muy cercanos al valor teórico de $V_{RMS}$ (2.031 V). Esto se debe a que el ancho de banda del osciloscopio (típicamente decenas o cientos de MHz) es muchísimo mayor que la frecuencia máxima probada (1 MHz), por lo que el instrumento puede reconstruir fielmente la forma de onda y calcular su valor RMS sin atenuación ni distorsión, sin importar la frecuencia de la señal.

El multímetro coincide razonablemente con el valor teórico a bajas frecuencias, pero se degrada drásticamente a partir de ciertos kHz. Entre 10 Hz y 1.7 kHz, tanto el TRMS (1.72--1.77 V) como el DC (1.69--1.77 V) se aproximan al valor teórico, con errores propios del tiempo de integración limitado de cada circuito de medición. A partir de 6 kHz, sin embargo, ambos canales colapsan hacia valores nulos, ya que el ancho de banda del multímetro, el cual está típicamente limitado a unos pocos kHz, actúa como un filtro pasa-bajos que ya no logra seguir la señal, ya que el conversor TRMS deja de responder correctamente, y el canal DC, el cual no tiene función True RMS, no logra separar el offset real de la componente AC de alta frecuencia, arrastrando la lectura hacia cero.

\subsection{Medición impedancia, comparación y desarrollo teórico del generador de señales}

El valor de impedancia de salida obtenido experimentalmente para el generador de señales Rigol DG4000 fue de 50.9 $\Omega$, mientras que el valor especificado por el fabricante en el datasheet es de 50 $\Omega$, lo que representa una diferencia de apenas 0.9 $\Omega$.

Este valor representa la resistencia interna equivalente del generador, la cual, según el modelo de Thévenin \cite{sedra2020}, se encuentra en serie con la fuente de voltaje ideal en el interior del equipo. Dicha impedancia determina cuánta tensión se pierde internamente antes de llegar a cualquier carga conectada a la salida, por lo que el generador no debe considerarse una fuente ideal sin resistencia interna, sino una fuente real que atenuará la señal en mayor o menor medida según la impedancia de entrada del dispositivo conectado.

El procedimiento se basa en un divisor de tensión resistivo formado por la impedancia de salida del generador ($Z_{out}$) y el trimmer ($R_{trimmer}$) conectados en serie:

\begin{equation}
V_{trimmer} = V_{gen} \cdot \frac{R_{trimmer}}{R_{trimmer} + Z_{out}}
\label{eq:divisor_trimmer}
\end{equation}

Al ajustar el trimmer hasta que la tensión observada en el osciloscopio se reduzca exactamente a la mitad de su valor original, de 4 Vpp a 2 Vpp, se cumple que:

\begin{equation}
V_{trimmer} = \frac{V_{gen}}{2}
\end{equation}

Sustituyendo esta condición en la ecuación del divisor de tensión y despejando:

\begin{equation}
\begin{aligned}
\frac{1}{2} &= \frac{R_{trimmer}}{R_{trimmer} + Z_{out}} \\
R_{trimmer} + Z_{out} &= 2R_{trimmer} \\
Z_{out} &= R_{trimmer}
\end{aligned}
\end{equation}

Esto demuestra que, cuando la tensión de salida se reduce exactamente a la mitad, la resistencia del trimmer es matemáticamente igual a la impedancia de salida del generador, ya que en esa condición ambas resistencias del divisor comparten la misma caída de tensión, situación que solo ocurre cuando tienen el mismo valor óhmico. Por ello, al desconectar el trimmer y medir su resistencia con el multímetro, se obtiene directamente el valor de $Z_{out}$.

\subsection{Medición impedancia, comparación y desarrollo teórico del osciloscopio}

La impedancia de entrada del osciloscopio ($Z_{in}$) representa la resistencia que el instrumento presenta a la señal conectada en su terminal de entrada, determinando cuánta corriente extrae del circuito bajo prueba. Un valor alto asegura que el osciloscopio no cargue significativamente el circuito medido. El valor obtenido experimentalmente da 994.4 k$\Omega$.

El circuito de la Figura 3 de la práctica forma un divisor de tensión entre $Z_{out}$ del generador (50 $\Omega$), el trimmer $R_1$ y $Z_{in}$ del osciloscopio:

\begin{equation}
V_{osc} = V_{gen} \cdot \frac{Z_{in}}{Z_{out} + R_1 + Z_{in}}
\label{eq:divisor_osc}
\end{equation}

Al ajustar $R_1$ hasta que la tensión caiga a la mitad, de 2 Vp a 1 Vp, se cumple:

\begin{equation}
V_{osc} = \frac{V_{gen}}{2}
\end{equation}

de donde:

\begin{equation}
Z_{in} = R_1 + Z_{out} \approx R_1
\end{equation}

ya que $Z_{out} = 50\ \Omega$ es despreciable frente a $R_1$ (del orden de M$\Omega$). Por ello, al medir el trimmer desconectado con el multímetro se obtiene directamente el valor de $Z_{in}$.

El valor medido, 994.4 k$\Omega$, está dentro de la tolerancia del fabricante de 1 M$\Omega \pm 1\%$, es decir, 990--1010 k$\Omega$, con un error relativo de 0.56\%, atribuible a la tolerancia del trimmer, la precisión del multímetro y la resolución visual al fijar el punto de 1 Vp en el osciloscopio.

\section{Conclusiones}
\begin{itemize}
    \item El modelo de Thévenin empleado para analizar los equipos de laboratorio (representado por las regiones punteadas y la resistencia interna $R_g$ en los diagramas) demuestra ser indispensable para predecir el comportamiento real de los instrumentos. Conocer la impedancia de salida del generador y la de entrada del osciloscopio permite evitar atenuaciones imprevistas o errores de medición inducidos por el efecto de carga \cite{sedra2020}.
    \item Al comparar los datos teóricos, simulados y experimentales del circuito resistivo, se evidencia una gran concordancia que ratifica el correcto uso del voltímetro y amperímetro, comprobando físicamente la validez de las Leyes de Kirchhoff y Ohm.
    \item La variación en la frecuencia permitió comprobar las limitaciones de ancho de banda inherentes a los multímetros. Mientras que el osciloscopio logra reconstruir fielmente los valores RMS sin atenuación hasta altas frecuencias (1 MHz), los multímetros colapsan sus lecturas a cero a partir de unos pocos kHz, lo que hace al osciloscopio la herramienta predilecta para el análisis de señales de alta frecuencia.
\end{itemize}
